...
class TeamleaderClient:
    ...
    def register_webhook(self, url, event_type):
        payload = {
            "url": self.secure_route(url),
            "types": [event_type]
        }
        return self.post_item('/webhooks.register', payload)

    def unregister_webhook(self, url, event_type, skip_jwtauth=False):
        if skip_jwtauth:
            hook_url = url
        else:
            hook_url = self.secure_route(url)

        payload = {
            "url": hook_url,
            "types": [event_type]
        }
        return self.post_item('/webhooks.unregister', payload)
